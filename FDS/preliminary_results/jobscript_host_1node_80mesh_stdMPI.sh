...
PY
    ...
    mpirun         --mca pml ob1         --mca btl self,vader,tcp         -np "$N"         hostname         > "$PLACEMENT_RAW"
    ...
    set +e
    mpirun         --mca pml ob1         --mca btl self,vader,tcp         -np "$N"         "$FDS_BIN" "$CASE_FILE"         > "$MAIN_LOG"         2> "$ERR_LOG"
    RC=$?
    set -e
